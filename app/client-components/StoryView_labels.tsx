...
import { TaskOrder, Epic, Story } from "./data-manipulation";
  ...
  async function handleScreenshot(e: any) {
    ...
    if (componentRef.current) {
      try {
        ...
        link.download = "story-view-screenshot.png";
        link.href = dataUrl;
        link.click();
      } catch (error) {
...
function StoryDisplay({
  ...
  if (!checkIfAnyLabelsAreInSelectedQuarters(selectedQuarters, story.labels)) {
    return <div></div>;
  }

  const gridRowIndex = index + 1;
  let gridRow = `${gridRowIndex} / ${gridRowIndex + 1}`;
  let gridColumn = getGridColumns(selectedQuarters, story.labels);
    ...
    >
      <HoverCard>
        <HoverCardTrigger asChild>
          <div className="text-gray-800 w-full ">{story.name}</div>
        </HoverCardTrigger>
        <HoverCardContent className="w-96">
          <div className="font-bold text-lg">{story.name}</div>
          <div>Planned Start: {story.plannedStart}</div>
          <div>Planned End: {story.plannedEnd}</div>
        </HoverCardContent>
      </HoverCard>
      <MileStones />
    </div>
