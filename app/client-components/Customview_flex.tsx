...
function ChartDisplay({
  ...
  const color = getShadeOfPurple(colorIndex);
  const firstColumn = "1";
...
function ItemDisplay({
  ...
  // Function to create a date without time component
  function createDateWithoutTime(date: Date | string): Date {
    const d = new Date(date);
    d.setHours(0, 0, 0, 0);
    return d;
  }

  const startDate = createDateWithoutTime(item.plannedStart);
  const endDate = createDateWithoutTime(item.plannedEnd);
  ...
  function isWithinDateRange(columnConfig: ColumnConfig) {
    ...
    const columnStartDate = createDateWithoutTime(columnConfig.startDate);
    const columnEndDate = createDateWithoutTime(columnConfig.endDate);
  ...
  function areColumnsAdjacent(col1: ColumnConfig, col2: ColumnConfig): boolean {
    if (!col1.endDate || !col2.startDate) return false;
    const col1EndDate = createDateWithoutTime(col1.endDate);
    const col2StartDate = createDateWithoutTime(col2.startDate);
